<div>
    <h4 class="mb-3">{{ __('settings_trans.manage_areas') }}</h4>
    ...
    <form wire:submit.prevent="{{ $updateMode ? 'update' : 'store' }}">
        ...
        <div class="mb-3">
            <label>{{ __('settings_trans.city') }}</label>
            <select class="form-control" wire:model="id_city">
                <option value="">{{ __('settings_trans.choose_city') }}</option>
                @foreach($cities as $c)

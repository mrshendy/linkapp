...
class application_settingsController extends Controller
{
    ...
    {
        return view('settings.splashscreen');
    }
    ...
    public function store(Storeapplication_settings $request)
    {
        if (Application_settings::where('name->ar', $request->name_ar)->orwhere('name->en', $request->name_en)->exists()) {
            return redirect()
                ->back()
                ->withErrors([trans('application_settings_trans.existes')]);
        }
        try {
    ...
    public function update(Storeapplication_settings $request)
    {
        try {
            ...
            $Application_settings->update([($Application_settings->name = ['en' => $request->name_en, 'ar' => $request->name_ar]), ($Application_settings->id_settings_type = $request->Id_settings_type), ($Application_settings->user_add = Auth::user()->id)]);
